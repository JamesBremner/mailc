...
bool cliutils::sync(IMAPConnection &imap) {
  ...
  std::transform(offline_mailboxes.begin(), offline_mailboxes.end(),
                 offline_mailboxes.begin(), tolowercase);

  // print_vectors(offline_mailboxes, '\n');
  std::cout << "Retrieving mailboxes" << std::endl;
  std::vector<std::string> mailboxes = imap.getmailboxes();
  std::transform(mailboxes.begin(), mailboxes.end(), mailboxes.begin(),
                 tolowercase);
  std::cout << "Retrieved mailboxes" << std::endl;
  // print_vectors(mailboxes);

  for (auto it = mailboxes.begin(); it != mailboxes.end(); it++) {
    if (std::find(offline_mailboxes.begin(), offline_mailboxes.end(), *it) ==
        offline_mailboxes.end()) {
  ...
  for (auto it = offline_mailboxes.begin(); it != offline_mailboxes.end();
       it++) {
  ...
  for (auto it = mailboxes.begin(); it != mailboxes.end(); it++) {
    ...
    for (auto it2 = offline_uids.begin(); it2 != offline_uids.end(); it2++) {
      if (std::find(uids.begin(), uids.end(), *it2) == uids.end()) {
        // Delete the mail
        std::string mailpath =
            MAIL_PATH() + mailbox + "/" + std::to_string(*it2) + ".m";
        fs::remove(mailpath);
      }
    }
    ...
    for (auto it2 = uids.begin(); it2 != uids.end(); it2++) {
      if (std::find(offline_uids.begin(), offline_uids.end(), *it2) ==
          offline_uids.end()) {
        std::string mailpath =
            MAIL_PATH() + mailbox + "/" + std::to_string(*it2) + ".m";
...
bool cliutils::deleteMail(IMAPConnection &imap, const std::string &mailbox,
                          int uid) {
  Mail mail;
  mail.uid = uid;
  mail.mailbox = mailbox;
  bool response = imap.deleteMail(mail);
  if (response) sync(imap);
  return response;
}

bool cliutils::moveMail(IMAPConnection &imap, const std::string &oldmailbox,
                        int uid, const std::string &newmailbox) {
...
std::vector<int> cliutils::searchMails(
    IMAPConnection &imap, const std::string &mailbox, const std::string &from,
...
bool cliutils::renameMailbox(IMAPConnection &imap,
                             const std::string &oldmailbox,
...
/**
 * @brief Random generator object
 */
std::mt19937 rng(rd());

bool cliutils::sendMail(config &config, const std::string &to,
  ...
  std::string sep = "------=_Part_" + std::to_string(uni(rng)) + "." +
                    std::to_string(uni(rng)) + std::to_string(uni(rng));
...
std::vector<Mail> cliutils::getMails(IMAPConnection &imap,
